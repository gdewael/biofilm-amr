...
def weights_plot(weights, X):
    fig_data = pd.DataFrame([weights.reshape(-1), np.tile(X.columns.values.astype(float), len(weights))*60]).T
    fig_data.columns = ["Weight", "Feature"]

    fig, axes = plt.subplots(1, 1)

    sns.lineplot(data = fig_data, x = "Feature", y = "Weight", ax = axes)
    axes.set_xlim([0, 75000])
    axes.set_xticks(ticks = np.arange(0, 75000, 5000))
    axes.set_xticklabels(labels = np.arange(0, 75000, 5000), rotation = 60)
    return fig, axes
...
def main():
    ...
    groups_CV = split_groups_CV(
        groups,
        ["Lineage", "Strain"],
        ["Lineage", "Selective pressure", "Strain"],
        print_ = True
    )
    ...
    for ix, (train_index, test_index) in enumerate(LeaveOneGroupOut().split(X, y, groups_CV)):
        ...
        for i in np.unique(groups_test):
            pred = model.predict(X_test.iloc[groups_test == i]).mean(0)
            true = y_test.values.reshape(-1)[groups_test == i][0]
    ...
    random_preds[np.isin(param_grid["all_labels"][0], np.unique(y_trues))] = (np.unique(y_trues, return_counts = True)[1] / len(y_trues))
