...
def weights_plot(weights, X):
    fig, axes = plt.subplots(1, 1, figsize=np.array((5,6)))
...
def main():
    ...
    parser.add_argument("WGS_preds_path", type=str, metavar="WGS_preds_path", help="path to WGS preds csv")
    parser.add_argument("MALDI_preds_path", type=str, metavar="MALDI_preds_path", help="path to MALDI preds csv")
    parser.add_argument("RAMAN_preds_path", type=str, metavar="RAMAN_preds_path", help="path to RAMAN preds csv")
    parser.add_argument("IMC_preds_path", type=str, metavar="IMC_preds_path", help="path to IMC preds csv")
    parser.add_argument("target", type=str, metavar="target", help="pred target. Either MIC or BPC")
    parser.add_argument(
    ...
    labelset = {
        "MIC" : np.array([1,2,4,8]),
        "BPC" : np.array([1,2,4,8,16,32]),
    }

    datasets = []
    for path, name in zip(
        [args.WGS_preds_path, args.MALDI_preds_path, args.RAMAN_preds_path, args.IMC_preds_path],
        ["WGS", "MALDI", "RAMAN", "IMC"],
    ):
    
        data = pd.read_csv(path, index_col=0)
        labels = data["True label"]
        preds = data.drop(["True label"], axis=1)
        preds_1D = (labelset[args.target].reshape(1, -1) * preds.values).sum(1)
        data = pd.DataFrame([labels.values, preds_1D]).T
        data.index = labels.index
        data.columns = [args.target, "Preds_%s" % name]
        datasets.append(data)
        if name == "WGS":
            groups = data.index


    print("Check if everything okay: ")
    print(print(all([all(datasets[0][args.target].values == d[args.target].values) for d in datasets])))

    input_data = np.log2(pd.concat([
        d.drop([args.target], axis=1).reset_index().drop(["index"], axis=1) for d in datasets
    ], axis=1)) / np.log2(labelset[args.target].max())
    input_data.index = groups

    X = input_data
    y = datasets[0][args.target]
